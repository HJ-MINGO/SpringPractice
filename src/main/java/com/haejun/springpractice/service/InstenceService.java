...
public class InstenceService {
     ...
     *
     *
     * 객체지향 + OCP, DIP를 가능하게 지원(DI컨테이너)
     *
     * 스프링이없더ㅏㄴ 시절에는 좋은 객체지향 개발을 위해 OCP,DIP원칙을 지키면서 개발을 해보니, 너무 할게 맣음
     *
     * 배보다 배꼽이 커서 프레임웍이 탄생
     *
     * 순수하게 자바로 OCP,DIP원칙들을 지키며 개발하게되면 결국 스프링 프레임워크가 된다. 그게바로 DI컨테이너 이다.
     *
     *
     * 실무의 고민
     * 인터페이스를 도입하면 추상화라는 비용이 발생한다.
     *
     * 그렇기 떄문에 설계때 확장가능성이 있는것은 인터페이스로
     *
     * 기능을 확장할 가능성이 없다면, 구체클래스를 직접 사용하고, 향후 꼭필요할 때 리팩터링 해서 인터페이스를 도입
     *
     * */
}
